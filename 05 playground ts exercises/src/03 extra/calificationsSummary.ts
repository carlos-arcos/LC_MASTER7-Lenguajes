...
const students = [
 ...
 const summarizeClassRoom = (studentList: Student[]):StudentSummary[]  => {
   ...
   for (let item of studentList) {
      const studentVM: StudentSummary = {
         name: item.name, 
         highestCalification: Math.max(...item.califications), 
         averageCalifications: (item.califications.reduce((a, b) => a + b, 0) / item.califications.length).toPrecision(3)
      };
      result.push(studentVM);
   }
